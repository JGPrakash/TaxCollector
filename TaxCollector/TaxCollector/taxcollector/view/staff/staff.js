...
$.fn.staffPage = function () {
    //https://jqueryvalidation.org/required-method/     for validation
    return $.pageController.getPage(this, function (page, $$) {
        
        page.staffServiceAPI = new StaffServiceAPI();
        document.title = "TaxCollector - Staff";
        page.id = null;
        ...
        page.events = {
            page_load: function () {
                ...
                setTimeout(function () {
                    $(".english-font").css("font", "Calibri !important");
                }, 2000)
            },
            btnSearch_click: function () {
                ...
                page.staffServiceAPI.searchValues(data.start_record, data.end_record, data.filter_expression, data.sort_expression, function (data) {
            ...
            btnSave_click: function () {
                try{
                    ...
                    if (data.name == "" || data.name == null || typeof data.name == "undefined") {
                        $$("txtName").focus();
                        throw WEBUI.LANG[CONTEXT.CurrentLanguage]["Staff name is mandatory ...!"];
                    }
                    ...
                    if (data.aadhar_no == "" || data.aadhar_no == null || typeof data.aadhar_no == "undefined") {
                        $$("txtAadharNo").focus();
                        throw "Aadhar no is mandatory ...!";
                    }
                    if (page.id == null || page.id == '') {
                        $$("msgPanel").show("Inserting new staff...");
                        page.staffServiceAPI.postValue(data, function (data) {
                            $$("msgPanel").show("New Staff saved successfully...!");
                            //Get the updated data
                            var data1 = {
                                id: data[0].key_value
                            }
                            page.staffServiceAPI.getValue(data1, function (data) {
                        ...
                        $$("msgPanel").show("Updating new staff...");
                        data.id = page.id;
                        page.staffServiceAPI.putValue(data.id, data, function (data1) {
                            $$("msgPanel").show("Staff updated successfully...!");
                            var data = {
                                id: page.id
                            }
                            page.staffServiceAPI.getValue(data, function (data) {
            ...
            btnDelete_click: function () {
                if (page.id == null || page.id == '')
                    $$("msgPanel").show("Select a staff first...!");
                else {
                    if (confirm(WEBUI.LANG[CONTEXT.CurrentLanguage]["Are You Sure Want To Remove This Staff If You Delete This Staff The Details Cannot Be Retrived"])) {
                        $$("msgPanel").show("Removing staff...");
                        var data = {
                            id: page.id
                        }
                        page.staffServiceAPI.deleteValue(page.id, data, function (data) {
                            $$("msgPanel").show("Staff removed successfully...!");
